/**
 * Хранилище «Сегодня» на устройстве (AsyncStorage).
 * Видимость: сегмент по workspaceId — stand-up и статусы видны участникам workspace (в проде — API/ACL).
 */
...
export async function loadBundle(workspaceId: string): Promise<TodayWorkspaceBundle> {
  const parsed = safeParse<TodayWorkspaceBundle>(await AsyncStorage.getItem(bundleKey(workspaceId)));
  if (!parsed) {
...
function saveBundle(workspaceId: string, bundle: TodayWorkspaceBundle) {
  void AsyncStorage.setItem(bundleKey(workspaceId), JSON.stringify(bundle)).catch(() => {});
}
